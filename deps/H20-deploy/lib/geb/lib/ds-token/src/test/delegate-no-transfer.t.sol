...
import "../delegate-no-transfer.sol";
...
contract TokenUser {
    DSDelegateTokenNoTransfer  token;

    constructor(DSDelegateTokenNoTransfer token_) public {
        token = token_;
    }
...
contract DSDelegateTokenNoTransferTest is DSTest {
    ...
    function createToken() internal returns (DSDelegateTokenNoTransfer) {
        return new DSDelegateTokenNoTransfer("TST", "TST");
    }
    ...
    function testWrongAccountTransfers() public logs_gas {
        uint sentAmount = 250;
        token.transferFrom(user2, self, sentAmount);
    }

    function testInsufficientFundsTransfers() public logs_gas {
        uint sentAmount = 250;
    ...
    function testTransferWithoutApproval() public logs_gas {
        token.transfer(user1, 50);
        token.transferFrom(user1, self, 1);
    }

    function testChargeMoreThanApproved() public logs_gas {
        token.transfer(user1, 50);
    ...
    function testTransferFromSelfNonArbitrarySize() public {
    ...
    function testBurnGuyWithoutTrust() public {
        uint burnAmount = 10;
    ...
    function testUntrustedTransferFrom() public {
        assertEq(token.allowance(self, user2), 0);
    ...
    function testPullWithoutTrust() public {
    ...
    function testMoveWithoutTrust() public {
    ...
    function testTransferAsNonTrustedCaller() public {
        token.mint(user1, 1);
    ...
    function testCheckpoints() public {
        token.mint(user1, 100); // user1 = delegator
    ...
    function testPriorVotesReturnsCorrectVotingBalance() public {
        ...
        assertEq(token.getPriorVotes(user2, 3), 100);
        assertEq(token.getPriorVotes(user3, 3), 0);
        assertEq(token.getPriorVotes(user2, 4), 100);
        assertEq(token.getPriorVotes(user2, 5), 100);
        assertEq(token.getPriorVotes(user3, 5), 0);
        assertEq(token.getPriorVotes(user2, 6), 100);
        assertEq(token.getPriorVotes(user2, 7), 120);
        assertEq(token.getPriorVotes(user3, 7), 0);
        assertEq(token.getPriorVotes(user2, 8), 120);
    }
}
